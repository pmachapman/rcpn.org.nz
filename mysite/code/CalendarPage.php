...
class CalendarPage_Controller extends Page_Controller {
	...
	public function GroupedCalendarEventsByDate() {
		...
		if ($view == "thisweek") {
			$sql = "SELECT `CalendarEvent`.*, DATE_FORMAT(`CalendarEventDate`.`Date`, '%W %e %M %Y') AS `Date` FROM `CalendarEvent` INNER JOIN `CalendarEventDate` ON `CalendarEventDate`.`CalendarEventID` = `CalendarEvent`.`ID` WHERE `Date` >= DATE_ADD(CURDATE(), INTERVAL (0 - WEEKDAY(CURDATE())) DAY) AND `Date` < DATE_ADD(CURDATE(), INTERVAL (7 - WEEKDAY(CURDATE())) DAY) ORDER BY `CalendarEventDate`.`Date`, `CalendarEvent`.`StartTime`, `CalendarEvent`.`EndTime`";
			...
			$sql = "SELECT `CalendarEvent`.*, DATE_FORMAT(`CalendarEventDate`.`Date`, '%W %e %M %Y') AS `Date` FROM `CalendarEvent` INNER JOIN `CalendarEventDate` ON `CalendarEventDate`.`CalendarEventID` = `CalendarEvent`.`ID` WHERE `Date` >= CONCAT(DATE_FORMAT(LAST_DAY(CURDATE()),'%Y-%m-'), '01') AND `Date` <= LAST_DAY(CURDATE()) ORDER BY `CalendarEventDate`.`Date`, `CalendarEvent`.`StartTime`, `CalendarEvent`.`EndTime`";
		} elseif ($view == "nextmonth") {
			$sql = "SELECT `CalendarEvent`.*, DATE_FORMAT(`CalendarEventDate`.`Date`, '%W %e %M %Y') AS `Date` FROM `CalendarEvent` INNER JOIN `CalendarEventDate` ON `CalendarEventDate`.`CalendarEventID` = `CalendarEvent`.`ID` WHERE `Date` >= CONCAT(DATE_FORMAT(LAST_DAY(DATE_ADD(CURDATE(), INTERVAL 1 MONTH),'%Y-%m-'), '01') AND `Date` <= LAST_DAY(DATE_ADD(CURDATE(), INTERVAL 1 MONTH)) ORDER BY `CalendarEventDate`.`Date`, `CalendarEvent`.`StartTime`, `CalendarEvent`.`EndTime`";
		} elseif ($view == "thisyear") {
			$sql = "SELECT `CalendarEvent`.*, DATE_FORMAT(`CalendarEventDate`.`Date`, '%W %e %M %Y') AS `Date` FROM `CalendarEvent` INNER JOIN `CalendarEventDate` ON `CalendarEventDate`.`CalendarEventID` = `CalendarEvent`.`ID` WHERE `Date` >= CONCAT(YEAR(CURDATE()), '-01-01') AND `Date` <= CONCAT(YEAR(CURDATE()), '-12-31') ORDER BY `CalendarEventDate`.`Date`, `CalendarEvent`.`StartTime`, `CalendarEvent`.`EndTime`";
		} elseif ($view == "nextyear") {
			$sql = "SELECT `CalendarEvent`.*, DATE_FORMAT(`CalendarEventDate`.`Date`, '%W %e %M %Y') AS `Date` FROM `CalendarEvent` INNER JOIN `CalendarEventDate` ON `CalendarEventDate`.`CalendarEventID` = `CalendarEvent`.`ID` WHERE `Date` >= CONCAT(YEAR(CURDATE()) + 1, '-01-01') AND `Date` <= CONCAT(YEAR(CURDATE()) + 1, '-12-31') ORDER BY `CalendarEventDate`.`Date`, `CalendarEvent`.`StartTime`, `CalendarEvent`.`EndTime`";
		} elseif ($view == "all") {
			$sql = "SELECT `CalendarEvent`.*, DATE_FORMAT(`CalendarEventDate`.`Date`, '%W %e %M %Y') AS `Date` FROM `CalendarEvent` INNER JOIN `CalendarEventDate` ON `CalendarEventDate`.`CalendarEventID` = `CalendarEvent`.`ID` WHERE `Date` >= CURDATE() ORDER BY `CalendarEventDate`.`Date`, `CalendarEvent`.`StartTime`, `CalendarEvent`.`EndTime`";
		} else {
			// Default to the next 14 days
			$sql = "SELECT `CalendarEvent`.*, DATE_FORMAT(`CalendarEventDate`.`Date`, '%W %e %M %Y') AS `Date` FROM `CalendarEvent` INNER JOIN `CalendarEventDate` ON `CalendarEventDate`.`CalendarEventID` = `CalendarEvent`.`ID` WHERE `Date` >= CURDATE() AND `Date` <= DATE_ADD(CURDATE(), INTERVAL 14 DAY) ORDER BY `CalendarEventDate`.`Date`, `CalendarEvent`.`StartTime`, `CalendarEvent`.`EndTime`";
		}
